...
import svc.BoardDriverListService;
import vo.ActionForward;
import vo.FileBoardDTO;
import vo.PageInfo;

public class BoardDriverListAction implements Action {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		System.out.println("BoardDriverListAction");
		...
		// BoardDriverListService 클래스의 인스턴스 생성 후
		// getListCount() 메서드를 호출하여 현재 file_board 테이블의 총 게시물 수 가져오기
		// => 파라미터 : 없음   리턴타입 : int(listCount)
		BoardDriverListService boardDriverListService = new BoardDriverListService();
		
		int listCount = boardDriverListService.getListCount(); // 전체 게시물 수
//		System.out.println("전체 게시물 수 : " + listCount);
		...
		// ----------------------------------------------------------------------------
		// 게시물 목록 가져오기 위해 BoardDriverListService 인스턴스의 getArticleList() 메서드 호출
		// => 파라미터 : 읽어올 페이지번호(pageNum), 조회할 목록 갯수(listLimit)
		//    리턴타입 : ArrayList<FileBoardDTO>(articleList)
		ArrayList<FileBoardDTO> articleList = 
				boardDriverListService.getArticleList(pageNum, listLimit);
...
//			int maxPage = (int)((double)listCount / listLimit + 0.9);
		...
		forward.setPath("./driver_list.jsp");
