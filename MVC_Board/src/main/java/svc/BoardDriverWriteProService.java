...
// Action 클래스에서 준비 작업을 통해 Service 클래스의 메서드를 호출하면
// 실제 비즈니스 로직 수행을 위해 DAO 객체에 작업을 요청하는 역할 = Service 클래스
// => 작업 과정에 따라 하나의 메서드 또는 복수개의 메서드를 정의
public class BoardDriverWriteProService {
	// 글쓰기 작업 요청을 위한 registArticle() 메서드 정의
	// => 파라미터 : Action 클래스로부터 전달받는 게시물 정보(FileBoardDTO 객체, fileBoard)
	// => 리턴타입 : boolean(isWriteSuccess)
	public boolean registArticle(FileBoardDTO fileBoard) {
		...
		//    FileBoardDAO 클래스의 getInstance() 메서드 호출
//		FileBoardDAO fileBoardDAO = new FileBoardDAO(); // 싱글톤 패턴 적용으로 인스턴스 직접 생성X
		...
		// => 글쓰기 작업을 위해 FileBoardDAO 객체의 insertFileBoard() 메서드 호출
