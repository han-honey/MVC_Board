...
ArrayList<FileBoardDTO> articleList = (ArrayList<FileBoardDTO>)request.getAttribute("articleList");
		...
		<article>
			...
			<table id="notice">
				...
				if(articleList != null && articleList.size() > 0) {
					for(FileBoardDTO fileBoard : articleList) {
						%>
						<tr onclick="location.href='./BoardDriverContent.bo?num=<%=fileBoard.getNum() %>&page=<%=pageNum%>'">
							<td><%=fileBoard.getNum() %></td>
							<td class="left">
								<!-- 답글 들여쓰기를 위해 들여쓰기 레벨(re_lev)에 따른 추가작업 -->
								<%if(fileBoard.getRe_lev() != 0) { %>
									<%for(int lev = 0; lev < fileBoard.getRe_lev(); lev++) {%>
										&nbsp;
									<%} %>
									▶
								<%} %>
								<%=fileBoard.getSubject() %>
							</td>
			...
			<div id="table_search">
				<input type="button" value="글쓰기" class="btn" 
						onclick="location.href='./BoardDriverWriteForm.bo'">
			</div>
			<div id="table_search">
			...
			<div id="page_control">
				...
				<%if(pageNum == startPage) { %>
					Prev&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<%} else { %>
					<a href="./BoardDriverList.bo?page=<%=pageNum - 1%>">Prev</a>
				<%} %>
				...
				<%for(int i = startPage; i <= endPage; i++) { %>
					...
					<%if(i == pageNum) { %>
						<%=i %>
					<%} else { %>
						<a href="./BoardDriverList.bo?page=<%=i%>"><%=i %></a>
					<%} %>
				<%} %>
				...
				<%if(pageNum == endPage) { %>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Next
				<%} else { %>
					<a href="./BoardDriverList.bo?page=<%=pageNum + 1%>">Next</a>
				<%} %>
			</div>
		</article>
